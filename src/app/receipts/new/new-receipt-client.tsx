...
import type { OcrAvailability } from "@/lib/deployment";

type Mode = "choose" | "qr" | "photo" | "manual";

export default function NewReceiptClient({ ocr }: { ocr: OcrAvailability }) {
  ...
  async function handlePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!ocr.available) {
      // Don't upload a photo to a route that has already said no.
      setInitialValues({ source: "PHOTO" });
      setOcrError(`${ocr.reason} Your photo is still attached — fill in the details below.`);
      setMode("manual");
      return;
    }
    ...
    try {
      ...
      setInitialValues({ source: "PHOTO" });
      // Two different things, said differently: a deployment that does not
      // offer automatic reading is not a photo this app failed to read.
      setOcrError(
        ocr.available
          ? "Could not read this photo automatically — fill in the details below."
          : `${ocr.reason} Your photo is still attached — fill in the details below.`,
      );
    } finally {
      setMode("manual");
    }
          ...
          <label className="rounded border px-4 py-3 text-left cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-900">
            ...
            {!ocr.available && (
              <span className="mt-1 block text-xs font-normal text-neutral-500">
                Automatic reading is off in this deployment — the photo is
                attached and the details are typed in.
              </span>
            )}
            <input
              type="file"
