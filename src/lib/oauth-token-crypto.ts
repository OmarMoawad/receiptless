...
/**
 * A real, valid 32-byte key generated once for local dev/CI, matching this
 * repo's other dev-default conventions. `?.trim() ||` rather than `??`
 * deliberately: a blank value in a copied .env means "not configured",
 * identical to unset — IDent hit exactly this bug (its API refused to boot
 * on a clean checkout) and it is not worth repeating here.
 */
const KEY_BASE64 = process.env.EMAIL_OAUTH_ENCRYPTION_KEY?.trim() || "Zx1pQ7yv3TgK8sHn2WdRfLmC5bJaVeXu9oPtYiUq0Ec=";
